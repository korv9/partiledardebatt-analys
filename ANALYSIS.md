...
- UMAP reducerar till 10 dimensioner inför HDBSCAN med `leaf`-urval, minsta gruppstorlek 250 och `min_samples=5`. En separat 2D-projektion används enbart för kartan. Kluster som i genomsnitt består av färre än 20 ord per segment räknas som kortfragment och lämnas ogrupperade. Om HDBSCAN ger färre än åtta grupper eller en grupp med över hälften av segmenten används 24-gruppers KMeans på de ursprungliga normaliserade embeddings. Den faktiskt använda metoden sparas i `data/features/metrics.json`.
...
- Klusterutvärderingen mäter cosinus-silhuett på ett fast urval av 2 400 segment i originalvektorerna; ogrupperade segment räknas separat som täckning. Den jämför inte koordinaterna i 2D. Känslighetskontrollen jämför närliggande HDBSCAN-parametrar på samma UMAP-projektion eller ett alternativt KMeans-startvärde. ARI ska inte tolkas som kvalitetsbetyg. Den mäter inte stabilitet mellan olika språkmodeller, tidsperioder eller UMAP-startvärden.
